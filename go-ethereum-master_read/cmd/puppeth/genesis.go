...
type cppEthereumGenesisSpec struct {
	...
	// 创世区块类，后面对应的是配置文件json字符串
	Genesis struct {
		Nonce      hexutil.Bytes  `json:"nonce"`      // 与mixHash结合用于满足POW条件
		Difficulty *hexutil.Big   `json:"difficulty"` // POW挖矿难度值
		MixHash    common.Hash    `json:"mixHash"`    // 与nonce结合用于满足POW条件
		Author     common.Address `json:"author"`
		Timestamp  hexutil.Uint64 `json:"timestamp"`  // 时间戳
		ParentHash common.Hash    `json:"parentHash"` // 父区块哈希
		ExtraData  hexutil.Bytes  `json:"extraData"`  // 与此区块相关的其他数据
		GasLimit   hexutil.Uint64 `json:"gasLimit"`   // 每个区块的Gas限制
	} `json:"genesis"`
